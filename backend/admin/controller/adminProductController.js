...
const validateDiscountDates = (discount) => {
  if (!discount || !discount.startDate || !discount.endDate) return null;
...
const getProductCategoriesForAdmin = async (_req, res) => {
  try {
    ...
    console.error('Admin get product categories error:', error);
...
const listProductsForAdmin = async (req, res) => {
  try {
    ...
    if (isDeleted !== undefined) query.isDeleted = toBoolean(isDeleted);
...
const createProductAsAdmin = async (req, res) => {
  try {
    ...
    if (!payload.seo.slug && payload.name) payload.seo.slug = toSlug(payload.name);
    if (!payload.createdBy) payload.createdBy = req.admin?._id;
...
const updateProductAsAdmin = async (req, res) => {
  try {
    ...
    if (updates.name) {
      ...
      if (!updates.seo.slug) updates.seo.slug = toSlug(updates.name);
    }

    if (updates.price !== undefined) updates.price = clampPrice(updates.price);
...
const restoreProductAsAdmin = async (req, res) => {
  try {
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res.status(400).json({ message: 'Invalid product id' });
